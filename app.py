...
# Design concept: a monochrome studio look, huge tracked type on true
# black, underline-only fields, no color accent. One grotesk family
# carries the whole page at different weights: 900 for the headline,
# 500 tracked caps for labels and section markers, 400 for body copy.
# No gradients, no shadows, no filled cards. White is the only accent.
CUSTOM_CSS = """
<style>
...
html, body, [class*="css"] { font-family: 'Archivo', -apple-system, sans-serif; }
h1, h2, h3, h4 { font-family: 'Archivo', sans-serif; color: var(--text); }

/* ---- Masthead ---- */
.masthead { padding: 0.6rem 0 2rem 0; margin-bottom: 1.8rem; border-bottom: 1px solid var(--border-soft); }
.wordmark {
    font-weight: 700; font-size: 0.85rem; letter-spacing: 0.14em;
    color: var(--text-muted); margin: 0 0 1.6rem 0;
}
.masthead h1 {
    font-size: 4.2rem; font-weight: 900; margin: 0; letter-spacing: -0.02em;
    line-height: 0.98; text-transform: uppercase;
}
.masthead p {
    font-family: 'Archivo', sans-serif; font-weight: 400; color: var(--text-muted);
    margin: 1rem 0 0 0; font-size: 1rem; max-width: 52ch;
}
...
section[data-testid="stSidebar"] h3 {
    font-family: 'Archivo', sans-serif; font-weight: 700; letter-spacing: 0.08em;
    font-size: 0.85rem; text-transform: uppercase; color: var(--text); margin-bottom: 0.4rem;
}
section[data-testid="stSidebar"] .stCaption, section[data-testid="stSidebar"] small {
    color: var(--text-muted);
}
section[data-testid="stSidebar"] a { color: var(--text); text-decoration: none; border-bottom: 1px solid var(--text-muted); }
section[data-testid="stSidebar"] a:hover { border-bottom-color: var(--text); }

/* ---- Inputs: underline only, no fill, no box ---- */
...
label, .stSlider label, .stCheckbox label p {
    color: var(--text-muted) !important; font-size: 0.82rem !important;
    text-transform: uppercase; letter-spacing: 0.06em; font-weight: 500;
}

/* Slider */
div[data-baseweb="slider"] div[role="slider"] { background: var(--text) !important; border-color: var(--text) !important; }
div[data-baseweb="slider"] > div > div:nth-child(2) { background: var(--text) !important; }
div[data-baseweb="slider"] > div > div:first-child { background: var(--border) !important; }

/* Checkbox */
[data-testid="stCheckbox"] svg { color: var(--text) !important; }
...
[data-testid="stDownloadButton"] button {
    ...
    border-radius: 2px; padding: 0.75rem 1.4rem; font-weight: 700; font-size: 0.85rem;
    letter-spacing: 0.08em; text-transform: uppercase; font-family: 'Archivo', sans-serif;
}
[data-testid="stDownloadButton"] button:hover { border-color: var(--text); background: var(--text); color: var(--bg); }
...
.section-label {
    ...
    letter-spacing: 0.08em; color: var(--text); font-size: 0.95rem; margin: 0 0 0.3rem 0;
    padding-bottom: 0.6rem; border-bottom: 1px solid var(--border-soft);
}
.section-meta { color: var(--text-muted); font-size: 0.82rem; margin: 0.6rem 0 1.1rem 0; }

/* ---- Output ---- */
.version-label {
    font-family: 'Archivo', sans-serif; font-weight: 700; text-transform: uppercase;
    letter-spacing: 0.08em; font-size: 0.78rem; color: var(--text-muted); margin: 1.3rem 0 0.6rem 0;
}
.output-card {
    background: transparent;
...
/* ---- Alerts: keep monochrome, no default blue/red fill ---- */
[data-testid="stAlert"] {
    background: transparent !important;
...
[data-testid="stAlert"] svg { fill: var(--text) !important; }
</style>
"""
...
# --------------------------------------------------------------------------
# Sidebar: API key (BYOK) and generation settings
# --------------------------------------------------------------------------
with st.sidebar:
    ...
    st.markdown("[Model list](https://console.groq.com/docs/models)")
...
st.markdown(
    """
    <div class="masthead">
        <p class="wordmark">AI CONTENT ASSISTANT</p>
        <h1>Content,<br>on brief.</h1>
        <p>Set the type, topic, audience and platform. Get a caption with hashtags, ready to post.</p>
    </div>
    """,
...
if "last_result" in st.session_state:
    ...
    st.markdown(
        f'<p class="section-meta">{meta["content_type"]} for {meta["platform"]}, generated {meta["generated_at"]}</p>',
        unsafe_allow_html=True,
    )
